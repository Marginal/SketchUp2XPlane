                ...
                if not texture.empty?
                  ...
                  if not material.texture
                    ...
                    if File.file?(texdir.join('/')+'/'+texture+orig_ext) && orig_ext.casecmp('.dds')==0
                      raise XPlaneImporterError, L10N.t("Can't read DDS files. Convert %s to PNG format") % (texdir.join('/')+'/'+texture+orig_ext)
                    else
                      raise XPlaneImporterError, L10N.t("Can't read texture file %s") % (texdir.join('/')+'/'+texture+orig_ext)
                    end
              ...
              when 'TRIS'
                ...
                while i<start+count
                  ...
                  begin
                    ...
                    msg[L10N.t("Ignoring some geometry that couldn't be imported")]=true if !(thisvt[0]==thisvt[1] || thisvt[0]==thisvt[2] || thisvt[1]==thisvt[2])	# SketchUp doesn't like colocated vertices or colinear faces -> <ArgumentError: Points are not planar>
                    i+=3	# next tri
                    next
                  end
